...
const store = createStore({
        ...
        actions: {
            ...
            //     }
            // },
            async enrollInCourse({ commit, state }, enrollData) {
                // Ensure enrolledCourses is initialized

                // Logic to enroll in course
                try {
                    console.log(enrollData);
                    await axios.post('http://localhost:8000/api/enroll/', enrollData); // Assuming an endpoint for enrolling
                    commit('SET_ENROLLED_COURSES', [...state.enrolledCourses, enrollData.course]); // Example mutation
                } catch (error) {
        ...
        getters: {
           ...
           ,
            ...
            ,
            getTeacherCoursesByUserId: (state) => (userId) => {
                const user = state.users.find(user => user.id === userId);
                if (!user) return [];
                return state.courses.filter(course => course.teachers.includes(user));
            },
            getCourseById:
                (state) => (id) => {
